...
void pab_unmap_page(struct device *dev, dma_addr_t dma_handle,
	...
	dma_unmap_single_attrs(__pab_get_parent(dev), dma_handle, size, dir, attrs);
}
...
void pab_unmap_sg(struct device *dev,
	...
	dma_unmap_sg_attrs(__pab_get_parent(dev), sg, nents, dir, attrs);
}

int pab_dma_supported(struct device *dev, u64 mask) {

	return dma_supported(__pab_get_parent(dev), mask);
}
...
static struct device_node *pab_fdt_load(const struct firmware *fdt){
	...
	of_fdt_unflatten_tree(dt_blob, NULL, &np);
	if (!np) {
		np = ERR_PTR(-ENODATA);
		goto out;
	}
