...
static const struct of_device_id fpga_overlay_of_match[] = {
...
static int __apply_overlay(struct fpga_overlay_dev *overlay_dev, struct device_node *overlay)
{
	...
	ret = of_platform_populate(overlay_dev->dev.of_node, fpga_overlay_of_match, NULL,&overlay_dev->dev);
	if (ret){
...
static int __fpga_overlay_apply(struct fpga_overlay_dev *overlay_dev)
{
	...
	of_fdt_unflatten_tree((void *)overlay_dev->fw->data, NULL, &overlay);
	if (!overlay) {
